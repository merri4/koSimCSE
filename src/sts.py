...
class NAISTSEvaluator(STSEvaluatorBase) :
    def __init__(self, path):
        dataset = pd.read_csv(path)
        sentences1 : list = list(dataset["sentence_1"])
        sentences2 : list = list(dataset["sentence_2"])
        scores : list = list(dataset["label"])
...
class STSEvaluation:
    def __init__(self, path):
        self.sts_evaluators = {
            "naists" : NAISTSEvaluator(path),
        }
        self.dev_evaluator = NAISTSEvaluator(path)
